...
def test_launch_layernorm_bw():
    ...
    def custom():
        inp_mt = minitorch.tensor(
            ln_input.clone().tolist(), backend=backend, requires_grad=True
        )
        gamma_mt = minitorch.tensor(
            gamma.clone().tolist(), backend=backend, requires_grad=True
        )
        beta_mt = minitorch.tensor(
            beta.clone().tolist(), backend=backend, requires_grad=True
        )
        out_mt = inp_mt.layernorm(gamma_mt, beta_mt)
        out_grad_mt = minitorch.tensor(
            out_grad.clone().tolist(), backend=backend, requires_grad=True
        )
    ...
    def baseline():
        f_input = minitorch.tensor(
            ln_input.clone().tolist(), backend=backend, requires_grad=True
        )
        f_gamma = minitorch.tensor(
            gamma.clone().tolist(), backend=backend, requires_grad=True
        )
        f_out_grad = minitorch.tensor(
            out_grad.clone().tolist(), backend=backend, requires_grad=True
        )
        ...
        f_stds = minitorch.tensor(
            np.sqrt(f_vars.to_numpy()).reshape(-1, 1).tolist(),
            backend=backend,
            requires_grad=True,
        )
