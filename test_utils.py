...
class TestDecorator(object):
    ...
    def case(self, dtypes=list(), ntest=2, nrepeat=2, rtol=1e-5, atol=1e-5):
        if not dtypes:
            dtypes = self.dtypes
    ...
    def test(self, custom, baseline, nrepeat, rtol, atol):
        ...
        def core(func):
            res, _ = func()  # warmup for GPU
            self.assert_allclose(res, res, rtol, atol)
            timing = list()
            for i in range(nrepeat):
                torch.cuda.synchronize(device=self.device)
                cur_res, cur_time = func()
                torch.cuda.synchronize(device=self.device)
                timing.append(cur_time)
