...
def get_dataset(dataset_name, model_max_length):
    ...
    dataset = {
        split: [
            ...
            if len(example[src_key].split()) + len(example[tgt_key].split())
            < model_max_length
        ]
...
def get_tokenizer(examples, vocab_size, src_key, tgt_key, workdir):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        workdir, eos_token=None, bos_token=None, pad_token=None, unk_token=None
    )
...
def collate_batch(examples, src_key, tgt_key, tokenizer, model_max_length, backend):
    ...
    label_token_weights = minitorch.tensor_from_numpy(
        label_token_weights, backend=backend
    )
...
def train(model, optimizer, examples, n_samples, collate_fn, batch_size, desc):
    ...
    for i in (
        prog_bar := tqdm.trange(0, len(examples), batch_size, desc=f"Training ({desc})")
    ):
...
def generate(
    model, examples, src_key, tgt_key, tokenizer, model_max_length, backend, desc
):
    """
    ...
    for example in tqdm.tqdm(examples, desc=f"Generating {desc}"):
        ...
        while len(token_ids) <= model_max_length:
            ...
            gen_id = 0
            logitcs = model.forward(Tensor(token_ids).view(1, len_src))
            gen_id = argmax(logitcs, dim=2)
            # END ASSIGN2_2
...
def main(
    ...
    for epoch_idx in range(n_epochs):
        ...
        json.dump(
            gen_examples, open(f"{workdir}/gen_epoch{epoch_idx}.json", "w"), indent=4
        )
