...
def GELU(input: Tensor) -> Tensor:
    ...
    return (
        0.5
        * input
        * (1 + (np.sqrt(2 / math.pi) * (input + 0.044715 * (input**3))).tanh())
    )
...
def one_hot(input: Tensor, num_classes: int) -> Tensor:
    ...
    return tensor_from_numpy(
        np.eye(num_classes)[input.to_numpy().astype(int)], backend=input.backend
    )
...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    """
    ### BEGIN YOUR SOLUTION
    # LSE(x1, x2, ... xn) = x_max + log(exp(x1 - x_max) + ... + expr(xn - x_max))
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    logits_target = (logits * one_hot_target).sum(dim=1)
    loss = lse - logits_target
