...
def test_embedding(batch_size, num_embeddings, seq_len, embedding_dim, backend):
    ...
    layer.weights.value = minitorch.tensor_from_numpy(
        layer_.weight.detach().numpy(), backend=backend, requires_grad=True
    )
...
def test_linear(sizes, bias, backend):
    ...
    layer_ = torch.nn.Linear(
        in_features=n, out_features=p, bias=bias, dtype=torch.float32
    )
    layer = minitorch.Linear(in_size=n, out_size=p, bias=bias, backend=backend)

    weights = layer_.weight.detach().numpy().T
    layer.weights.value = minitorch.tensor_from_numpy(
        weights.copy(), backend, requires_grad=True
    )
    if bias:
        b = layer_.bias.detach().numpy()
        layer.bias.value = minitorch.tensor_from_numpy(
            b.copy(), backend, requires_grad=True
        )
    ...
    np.testing.assert_allclose(
        result.to_numpy(), result_.detach().numpy(), rtol=1e-5, atol=1e-5
    )
    ...
    np.testing.assert_allclose(
        X.grad.to_numpy(), X_.grad.detach().numpy(), rtol=1e-5, atol=1e-5
    )
...
def test_linear_double(sizes, bias, backend):
    ...
    layer_1_ = torch.nn.Linear(
        in_features=n_embd, out_features=middle_dim, bias=bias, dtype=torch.float32
    )
    layer_2_ = torch.nn.Linear(
        in_features=middle_dim, out_features=n_embd, bias=bias, dtype=torch.float32
    )
    layer_1 = minitorch.Linear(
        in_size=n_embd, out_size=middle_dim, bias=bias, backend=backend
    )
    layer_2 = minitorch.Linear(
        in_size=middle_dim, out_size=n_embd, bias=bias, backend=backend
    )

    weights_1 = layer_1_.weight.detach().numpy().T
    weights_2 = layer_2_.weight.detach().numpy().T
    layer_1.weights.value = minitorch.tensor_from_numpy(
        weights_1.copy(), backend, requires_grad=True
    )
    layer_2.weights.value = minitorch.tensor_from_numpy(
        weights_2.copy(), backend, requires_grad=True
    )
    if bias:
        b_1 = layer_1_.bias.detach().numpy()
        layer_1.bias.value = minitorch.tensor_from_numpy(
            b_1.copy(), backend, requires_grad=True
        )
        b_2 = layer_2_.bias.detach().numpy()
        layer_2.bias.value = minitorch.tensor_from_numpy(
            b_2.copy(), backend, requires_grad=True
        )
...
def test_layernorm(batch_size, dim, eps, backend):
    ...
    layer_ = torch.nn.LayerNorm(normalized_shape=dim, eps=eps)
    ...
    np.testing.assert_allclose(
        x_minitorch.grad.to_numpy(), x_torch.grad.detach().numpy(), atol=1e-5, rtol=1e-5
    )
