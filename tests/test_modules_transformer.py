...
def test_multihead_attention(
    ...
    layer = minitorch.MultiHeadAttention(
        n_embd, num_heads, True, p_dropout, bias=False, backend=backend
    )
...
def test_feedforward_layer(batch_size, seq_len, n_embd, dropout, backend):
    ...
    x = np.random.randn(batch_size, seq_len, n_embd).astype(datatype)
...
def test_transformer_layer(
    batch_size, seq_len, n_embd, num_heads, causal, p_dropout, ln_eps, bias, backend
):
    """
    ...
    # Transformer Weights
    w_qkv = (
        layer_.self_attn.in_proj_weight.detach().numpy().T.copy()
    )  # (n_embd, 3*n_embd)
    w_q_, w_k_, w_v_ = [
        w.copy() for w in np.split(w_qkv, 3, -1)
    ]  # 3 * (n_embd, n_embd)
    ...
    layer.attention.q_projection.weights.value = minitorch.tensor_from_numpy(
        w_q_, backend=backend, requires_grad=True
    )
    layer.attention.k_projection.weights.value = minitorch.tensor_from_numpy(
        w_k_, backend=backend, requires_grad=True
    )
    layer.attention.v_projection.weights.value = minitorch.tensor_from_numpy(
        w_v_, backend=backend, requires_grad=True
    )
    layer.attention.out_projection.weights.value = minitorch.tensor_from_numpy(
        w_out_, backend=backend, requires_grad=True
    )
    layer.ff.linear_in.weights.value = minitorch.tensor_from_numpy(
        w_ffn_in, backend=backend, requires_grad=True
    )
    layer.ff.linear_out.weights.value = minitorch.tensor_from_numpy(
        w_ffn_out, backend=backend, requires_grad=True
    )
