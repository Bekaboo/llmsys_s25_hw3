...
def test_cuda_matmul_numpy_eq(m, n, p, backend):
    ...
    np.testing.assert_allclose(c.to_numpy(), _c, atol=1e-5, rtol=1e-5)


@pytest.mark.parametrize("backend", matmul_tests)
def test_cuda_matmul_transpose(backend: str) -> None:
    ...
    np.testing.assert_allclose(
        z.to_numpy(), np.array(x1).T @ np.array(y1), atol=1e-5, rtol=1e-5
    )
